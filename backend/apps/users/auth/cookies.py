...
def set_trusted_device_cookie(response, raw_token):
    ...
    samesite = settings.SIMPLE_JWT.get("AUTH_COOKIE_SAMESITE", "Lax")
    cookie_path = settings.SIMPLE_JWT.get("AUTH_COOKIE_PATH", "/")
    max_age = int(getattr(settings, "AUTH_TRUSTED_DEVICE_DAYS", 30) * 24 * 60 * 60)
...
def clear_auth_cookies(response):
    cookie_path = settings.SIMPLE_JWT.get("AUTH_COOKIE_PATH", "/")
    response.delete_cookie(settings.SIMPLE_JWT["AUTH_COOKIE"], path=cookie_path)
    response.delete_cookie(settings.SIMPLE_JWT["AUTH_COOKIE_REFRESH"], path=cookie_path)
    return response
