...
class PredefinedSchemaColumnForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        config = SCHEMA_COLUMN_CONFIG[source][source_field]
...
class StockPortfolioSchemaColumnAdminForm(forms.ModelForm):
    predefined_column = forms.ChoiceField(
        ...
        required=False,
        label="Predefined Column",
        help_text="Select a predefined column to auto-fill below fields.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Generate choices from SCHEMA_COLUMN_CONFIG
        predefined_choices = []
        for source, fields in SCHEMA_COLUMN_CONFIG.items():
            for field, config in fields.items():
                label = f"{source} - {field}" if field else f"{source} - custom"
                predefined_choices.append((f"{source}:{field}", label))

        self.fields['predefined_column'].choices = [
            ('', '---------')] + predefined_choices

        # If selected, prefill other fields
        predefined_key = self.data.get(
            'predefined_column') or self.initial.get('predefined_column')
        if predefined_key and ':' in predefined_key:
            source, field = predefined_key.split(':', 1)
            config = SCHEMA_COLUMN_CONFIG.get(source, {}).get(field)

            if config:
                self.initial.setdefault('source', source)
                self.initial.setdefault('source_field', field)
                self.initial.setdefault('data_type', config.get('data_type'))
                self.initial.setdefault(
                    'editable', config.get('editable', True))
                self.initial.setdefault('formula', config.get('formula', ''))

        # Optional: hide 'formula' field if not needed
        if not self.initial.get('formula'):
...
class StockPortfolioSchemaColumnValueAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'column' in self.data:
            ...
            try:
                column = StockPortfolioSchemaColumn.objects.get(
...
class StockPortfolioSchemaAdmin(admin.ModelAdmin):
    ...
    def stock_portfolio_link(self, obj):
        url = reverse('admin:stock_portfolio_stockportfolio_change',
